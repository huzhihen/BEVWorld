...
from mmdet3d.core import limit_period, bbox
...
class AnchorTrainMixin(object):
    ...
    def anchor_target_3d_single(self,
        ...
        """
        # print(f'@@@@@@@@@@@@@mmdet3d/models/dense_heads/train_mixins.py:126 type(anchors)={type(anchors)} anchors.shape={anchors.shape}')
        # print(f'@@@@@@@@@@@@@mmdet3d/models/dense_heads/train_mixins.py:127 type(gt_bboxes)={type(gt_bboxes)} anchors.shape={gt_bboxes.tensor.shape if type(gt_bboxes) is not list else list([box.tensor.shape for box in gt_bboxes])}')
        # print(f'@@@@@@@@@@@@@mmdet3d/models/dense_heads/train_mixins.py:128 type(gt_labels)={type(gt_labels)} anchors.shape={gt_labels.shape if type(gt_labels) is not list else list([label.shape for label in gt_labels])}')
        
        # print(f'@@@@@@@@@@@@@mmdet3d/models/dense_heads/train_mixins.py:133 type(self.bbox_assigner)={type(self.bbox_assigner)} type(anchors)={type(anchors)}')
        ...
        if isinstance(self.bbox_assigner,
            ...
            for i, assigner in enumerate(self.bbox_assigner):
                ...
                if self.assign_per_class:
                    ...
                    # print(f'@@@@@@@@@@@@@mmdet3d/models/dense_heads/train_mixins.py:147 gt_per_cls={gt_per_cls} {type(gt_per_cls)}')
                    # print(f'@@@@@@@@@@@@@mmdet3d/models/dense_heads/train_mixins.py:148 gt_bboxes={len(gt_bboxes)}:{type(gt_bboxes[0])}')
                    # print(f'@@@@@@@@@@@@@mmdet3d/models/dense_heads/train_mixins.py:149 gt_bboxes={gt_bboxes[0][gt_per_cls, :]}')
    ...
    def anchor_target_single_assigner(self,
        ...
        if len(gt_bboxes) > 0:
            # print(f'@@@@@@@@@@@@@mmdet3d/models/dense_heads/train_mixins.py:280 len(gt_bboxes)={len(gt_bboxes)}')
            # raise RuntimeError()
