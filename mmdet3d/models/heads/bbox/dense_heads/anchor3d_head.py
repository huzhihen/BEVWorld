...
from mmdet3d.core import (PseudoSampler, box3d_multiclass_nms, limit_period, bbox,
                          xywhr2xyxyr)
...
class Anchor3DHead(BaseModule, AnchorTrainMixin):
    ...
    def loss_single(self, cls_score, bbox_pred, dir_cls_preds, labels,
        ...
        if torch.isnan(cls_score).any(): 
            print("WARNING: cls_score contains NaN!")
            raise ValueError("cls_score contains NaN!")
        if torch.isinf(cls_score).any(): 
            print("WARNING: cls_score contains Inf!")
            raise ValueError("cls_score contains Inf!")

        loss_cls = self.loss_cls(
            cls_score, labels, label_weights, avg_factor=num_total_samples)
        # print(f'loss_cls: {loss_cls.shape} {loss_cls}')
        # regression loss
        bbox_pred = bbox_pred.permute(0, 2, 3,
                                      1).reshape(-1, self.box_code_size)
        bbox_targets = bbox_targets.reshape(-1, self.box_code_size)
        bbox_weights = bbox_weights.reshape(-1, self.box_code_size)
        #print(f'bbox_pred: {bbox_pred.shape} {bbox_pred}')
        #print(f'bbox_targets: {bbox_targets.shape} {bbox_targets}')
        #print(f'bbox_weights: {bbox_weights.shape} {bbox_weights}')
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        """Loss function for Anchor3DHead.
        Args:
        ...
        """
        cls_scores, bbox_preds, dir_cls_preds = preds_dicts
        gt_bboxes = gt_bboxes_3d
        gt_labels = gt_labels_3d
        input_metas = kwargs
        gt_bboxes_ignore = None
        if input_metas is None or len(input_metas) == 0:
            input_metas = self.metas
        # print(f'input_metas: {input_metas}')
        return self._loss(cls_scores, bbox_preds, dir_cls_preds, gt_bboxes, gt_labels, input_metas, gt_bboxes_ignore)
        return None
    
    # @auto_fp16(apply_to=('cls_scores', 'bbox_preds', 'dir_cls_preds'))
    def _loss(self,
             cls_scores,
             bbox_preds,
             dir_cls_preds,
        ...
        assert len(featmap_sizes) == self.anchor_generator.num_levels, (len(featmap_sizes), self.anchor_generator.num_levels)
        ...
        return dict(
            loss_cls=losses_cls[0], loss_bbox=losses_bbox[0], loss_dir=losses_dir[0])
    ...
    def get_bboxes(self,
        ...
        if input_metas is None or len(input_metas) == 0:
            input_metas = self.metas
        #print(f'input_metas: {input_metas}')
        return self._get_bboxes(cls_scores, bbox_preds, dir_cls_preds, input_metas, cfg, rescale)
